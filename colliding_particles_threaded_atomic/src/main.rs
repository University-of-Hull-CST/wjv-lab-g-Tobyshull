...
const COLLISION_THREAD_COUNT : usize = 10;
...
fn collision_thread_main(particle_system: Arc<Mutex<ParticleSystem>>, counter: Arc<AtomicUsize>) {
    let start_time = Instant::now();
    ...
    counter.fetch_add(collision_counter, Ordering::Relaxed);
}
...
fn main() {
    ...
    let collision_count = Arc::new(AtomicUsize::new(0)); // Use an atomic usize to allow for safe thread management
    ...
    for i in 0..COLLISION_THREAD_COUNT {
        let system_clone = Arc::clone(&particle_system_mut);
        let counter_clone = Arc::clone(&collision_count);

        collision_pool.execute(move || collision_thread_main(system_clone, counter_clone));
    }
    ...
    println!("{} collisions occured", collision_count.load(Ordering::Relaxed));
}
